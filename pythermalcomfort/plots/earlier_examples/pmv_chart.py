...
def pmv_chart(df, pmv_constants, show_summary=True, si_ip="si"):
    ...
    # fig.add_trace( #! remove neutral line, concept does not exist in ASHRAE 55
    #     go.Scatter(
    #         x=neutral_line[0],
    #         y=neutral_line[1],
    #         mode="lines",
    ...
    if show_summary:
        ...
        df.loc[df["pmv"] > 0.5, "categorical"] = "Warmer than<br>Comfortable"
        df.loc[df["pmv"] < -0.6, "categorical"] = "Cooler than<br>Comfortable"
        df.loc[df["pmv"].between(-0.5, 0.5), "categorical"] = "Comfortable"
        df.loc[pd.isna(df["pmv"]), "categorical"] = "Out of <br>Applicable Range"
