...
class IREQInputs(BaseInputs):
    def __init__(
        self,
        m: float | int | np.ndarray | list,
        w_work: float | int | np.ndarray | list = 0,
        tdb: float | int | np.ndarray | list = None,
        tr: float | int | np.ndarray | list = None,
        p_air: float | int | np.ndarray | list = 8.0,
        v_walk: float | int | np.ndarray | list = 0.3,
        v: float | int | np.ndarray | list = 0.4,
        rh: float | int | np.ndarray | list = 50.0,
        clo: float | int | np.ndarray | list = 1.0,
        round_output: bool = True,
        limit_inputs: bool = True,
    ):
        # Initialize with IREQ specific parameters
        super().__init__(
            tdb=tdb,
            tr=tr,
            v=v,
