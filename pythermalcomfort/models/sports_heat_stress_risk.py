...
if __name__ == "__main__":
    ...
    t = 36
    rh = 40
    r = sports_heat_stress_risk(tdb=t, rh=rh, tr=t + 10, sport=Sports.RUNNING, vr=1.0)
    print(r)

    results = []
    for t, rh in product(np.arange(25, 45, 1), range(0, 101, 2)):
        risk_value = sports_heat_stress_risk(
            tdb=t, rh=rh, tr=t + 10, sport=Sports.RUNNING, vr=1.0
        )
        results.append((t, rh, risk_value.risk_level_interpolated))

    df = pd.DataFrame(results, columns=["tdb", "rh", "risk"]).astype(
        {"tdb": float, "rh": float, "risk": float}
    )
    # round the risk to math floor integer values for the heatmap
    df["risk"] = df["risk"].apply(lambda x: int(np.floor(x)))
