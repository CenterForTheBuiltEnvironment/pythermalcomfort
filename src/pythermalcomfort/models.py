...
# import functions for Zhang's sensation and comfort model
from pythermalcomfort.zhang_comfort_model_functions import utilities, sensation, comfort
...
def zhang_sensation_comfort(
    ...
    """Zhang's local and overall sensation and comfort model.
        ...
        >>> print(dict_results)
        {
        'sensation_overall': -0.65,
        'comfort_overall': -2.52,
        'sensation_head': 0.15,
        'sensation_neck': 0.15,
        'sensation_chest': -3.0,
        'sensation_back': -3.0,
        'sensation_pelvis': -3.0,
        ...
        'comfort_back': -2.43,
        'comfort_pelvis': -2.7,
        'comfort_left_shoulder': 1.26,
        'comfort_left_arm': 2.29,
        'comfort_left_hand': 1.79,
        'comfort_right_shoulder': 1.26,
        'comfort_right_arm': 2.29,
        'comfort_right_hand': 1.79,
        'comfort_left_thigh': -1.36,
        'comfort_left_leg': 0.5,
        'comfort_left_foot': 1.56,
        'comfort_right_thigh': -1.36,
        'comfort_right_leg': 0.5,
        'comfort_right_foot': 1.56
    }
    """
    default_options = {
    ...
    t_skin_local_array = utilities.convert_to_17_segments_array_from_any_data_type(
        t_skin_local
    )
    dt_skin_local_dt_array = utilities.convert_to_17_segments_array_from_any_data_type(
        dt_skin_local_dt
    )
    t_skin_local_set_array = utilities.convert_to_17_segments_array_from_any_data_type(
        t_skin_local_set
    )
    ...
    dt_core_local_dt_array = utilities.convert_to_17_segments_array_from_any_data_type(
        dt_core_local_dt
    )
    ...
    sensation_local_array = sensation.calculate_local_thermal_sensation(
    ...
    overall_sensation_value = sensation.calculate_overall_sensation(
        sensation_local_array=sensation_local_array
    )

    # Calculate local thermal comfort
    comfort_local_array = comfort.calculate_local_thermal_comfort(
        overall_sensation=overall_sensation_value,
        sensation_local_array=sensation_local_array,
    )

    # Calculate overall thermal comfort
    overall_comfort_value = comfort.calculate_overall_thermal_comfort(
    ...
    for key in dict_data.keys():
        if "local" in key:
            ...
            for i, body_name in enumerate(utilities.body_name_list):
    ...
    for key in list(dict_data.keys()):
        if "local" in key and not any(part in key for part in utilities.body_name_list):
            del dict_data[key]
...
class ATCS(JOS3):
    ...
    def __init__(self, *args, **kwargs):
        ...
        print(f"During initialization, overall clo value is {self.overall_clo}")
    ...
    def _calculate_comfort_setpt(self, overall_clo, par):
        ...
        # Otherwise, calculate a new comfortable skin temperature set point
            ...
            print("Skin neutral temperature calculation starts")
            print("Metabolic rate:", met)
            print("Overall clo value:", overall_clo)
            print("Operative temperature when PMV=0", self.to)
            for i in range(10):
                self._run(dtime=600, passive=False)

            self.comfort_setpt_skin = self.t_skin
            print("Comfort set-point temperature:", self.comfort_setpt_skin)
